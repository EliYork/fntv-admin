...
默认宿主机端口是 `18080`，容器内部端口保持 `8080`。需要修改宿主机端口时，在 `.env` 中设置 `FNTV_ADMIN_PORT`，例如 `FNTV_ADMIN_PORT=18081`。
...
不同飞牛版本中 `watched` 可能表现为看完标记，也可能看起来像累计时长。系统诊断会展示最小值、最大值、取值样本和非零数量，且不返回真实播放记录行。存在明确 `watched/completed` 字段时以该字段为准：完成记录即使 position 归零也显示“已看完”；只有 Schema 没有完成字段时，才按 position 达到 runtime 推断完成。观看历史、数据中心和 report API 使用同一口径。

## 公网访问安全吗？

项目默认面向内网 NAS。公网访问必须使用 HTTPS 反向代理、强密码、登录失败限制，并且不要直接暴露未加保护的管理后台。未初始化实例仍要求 `/data/init-admin.token` 中的一次性部署凭据，远程访问者仅凭网页无法抢先创建管理员。

## 首次初始化凭据在哪里？

应用未初始化时会在 `/data/init-admin.token` 生成强随机一次性凭据。在 Compose 文件所在目录运行 `docker compose exec fntv-admin cat /data/init-admin.token`，把结果填入首次初始化页面。管理员创建成功后文件立即删除；不要把它写进 Compose、`.env`、截图或普通日志。
...
默认不信任 `Forwarded` / `X-Forwarded-For` / `X-Real-IP`。反向代理场景必须同时设置 `TRUST_PROXY_HEADERS=true` 与精确的 `TRUSTED_PROXIES` IP/CIDR，并让代理覆盖或正确追加来源头。只有直接连接应用的 peer 属于可信代理时才解析代理链；伪造头、无效链或来源不确定都不会获得本地免登录。不要使用 `0.0.0.0/0` 或 `::/0`。
...
数据中心使用 stale-while-revalidate。切换范围或手动刷新时，页面会保留上次成功数据，同时显示“正在更新”。如果新请求失败，页面只在对应模块显示“更新失败，仍显示上次成功数据”，不会清空旧表格或热力图，也不会弹出误导性的登录错误。旧 `/reports` 链接会重定向到数据中心。
