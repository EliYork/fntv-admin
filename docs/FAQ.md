...
项目默认面向内网 NAS。公网访问必须使用 HTTPS 反向代理、强密码、登录失败限制，并且不要直接暴露未加保护的管理后台。首次安装建议先通过可信局域网访问 NAS 地址完成管理员创建，再配置公网映射或反向代理；尚未创建管理员的实例如果直接暴露到不可信网络，存在被第三方抢先创建管理员的潜在风险。

## 首次创建管理员需要额外部署步骤吗？

不需要。全新安装启动后，通过可信局域网打开管理页面，填写用户名和密码即可创建首个管理员。管理员创建完成后，初始化接口会永久拒绝再次创建。
