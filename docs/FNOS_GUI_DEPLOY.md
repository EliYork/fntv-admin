...
飞牛 NAS 默认推荐使用 Docker Hub 成品镜像部署 `fntv-admin`，不要在飞牛本机 build 镜像。GHCR 作为备用镜像源。

原因：

- 飞牛本机构建时可能需要拉取 `docker.io/library/node:22-alpine` 和 `docker.io/library/python:3.12-slim`。
- 这些基础镜像在部分网络环境下容易超时。
- GitHub Actions 可以在云端完成多阶段构建，并把最终单容器镜像推送到 Docker Hub 与 GHCR。
- Docker Hub 是默认镜像源；GHCR 用于备用拉取。

## 部署方式 A：导入 Compose / 项目 / 应用栈

在飞牛 Docker 可视化界面中选择 Compose、项目或应用栈导入，把项目的 `docker-compose.yml` 内容粘贴进去。

默认部署使用 Docker Hub 镜像：

```yaml
image: docker.io/eliyork/fntv-admin:latest
```

如果 Docker Hub 在当前网络下不可用，可以改用 GHCR 备用镜像：
...
默认本地访问和外部访问都需要登录。登录系统会保留，不建议直接公开无保护后台。

在系统设置页可以切换访问控制：

```text
飞牛内网自用推荐：本地访问免登录，外部访问需要登录
公网 / DDNS / 反向代理推荐：本地访问需要登录，外部访问需要登录或禁止访问
```
...
如果 Docker Hub 镜像拉取失败，请检查：

1. 飞牛 NAS 是否能访问 `docker.io`。
2. 镜像名是否为 `docker.io/eliyork/fntv-admin:latest` 或稳定版本 tag。
3. 本地网络或镜像代理是否阻断 Docker Hub。

如果 Docker Hub 在当前网络下不可用，可以使用 GHCR 备用镜像：
