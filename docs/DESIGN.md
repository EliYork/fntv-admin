...
5. 当前提供仪表盘（数据中心）、观看历史、用户管理、媒体库、系统设置、系统诊断等模块；报表统计能力已整合进数据中心，任务中心和日志中心作为后续维护能力规划，不在当前 UI 中暴露入口。
...
GET /api/reports/play-trend?days=365
GET /api/reports/top-users?days=30&limit=10
GET /api/reports/top-media?days=30&limit=10
GET /api/reports/hourly-distribution?days=30
GET /api/reports/media-type-distribution
GET /api/reports/resolution-distribution?days=30
GET /api/favorites?page=1&page_size=20&keyword=
GET /api/downloads?page=1&page_size=20&keyword=&status=
```

Phase 7A/7B/7C 的报表 API 均需要登录鉴权，只读读取飞牛源库或可选快照，不写飞牛数据库。播放趋势、活跃用户、热门媒体、播放时段和分辨率分布在后端 SQL/服务层按时间范围聚合；前端播放趋势固定请求最近 365 天，榜单、播放时段和分辨率分布分别维护自己的 7 / 30 / 90 天或全部周期，热门媒体支持 `episode` / `series` 统计模式，服务层会限制最大天数和最大 `limit`。收藏记录读取 `item_user_favorite`，下载记录读取 `download_task`，缺表时返回空状态和能力不可用。更复杂的完播率排行、弃剧排行和缓存预计算留到后续阶段。
...
数据中心是日常使用入口，高频统计与连续观看历史集中在同一页面。项目不使用固定左侧导航栏；低频页面由顶部“功能与设置”入口打开抽屉后访问，避免重新引入常驻侧栏。
...
1. 仪表盘是单页数据中心，不使用固定侧栏，不在本页堆叠低频管理入口。
2. 首页沿用砖红、暖灰主题变量和轻量信息层级，不使用默认蓝色监控面板或玻璃拟态视觉。
3. 继续复用现有登录、功能抽屉、API 鉴权和只读查询链路。

模块：

1. 四个总览指标：总用户数、活跃用户、今日播放、播放记录。
2. 播放趋势日期点阵固定展示最近 1 年，不提供周期切换和颜色图例。
3. 播放时段使用轻量柱状图，默认 30 天，可独立选择 7 / 30 / 90 天或全部。
4. 热门内容默认 7 天，可独立选择 7 / 30 / 90 天或全部。
5. 活跃用户默认 30 天，可独立选择 7 / 30 / 90 天或全部。
6. 观看历史连续加载，按日期分组，并保留独立筛选与导出能力。

容错：

1. 首页各模块独立显示错误或空状态，单个接口失败不导致整页白屏。
2. 刷新失败时保留上次成功数据。
3. 仪表盘首页不写飞牛数据库，不写入 `admin.db`，只复用现有只读 API。
...
1. 播放历史表格。
2. 用户筛选。
3. 标题搜索，包含当前媒体、父级和祖父级媒体标题，便于用剧名命中单集播放记录。
4. 时间范围筛选。
5. 是否看完筛选。
6. 分辨率筛选。
7. 分页。
...
旧 `/reports` 路径只保留兼容重定向到 `/dashboard`，功能抽屉不再提供独立报表中心入口。后端 `/api/reports/*` 与 report service 保留，继续供数据中心使用。
...
1. 报表总览。
2. 播放趋势日期热力图按周为列、星期为行固定展示最近 1 年；不提供趋势周期切换。
3. 活跃用户榜默认 30 天，周期独立可选 7 / 30 / 90 天或全部。
4. 热门媒体榜默认 7 天，周期独立可选 7 / 30 / 90 天或全部，并支持单集/整部剧统计模式。
5. 媒体类型分布，中文展示。
6. 分辨率分布，缺失值显示为“未记录”。
7. 报表模块使用 stale-while-revalidate：刷新和切换范围时保留上次成功数据，显示“正在更新”和最后更新时间；更新失败时只在模块内提示。
8. 播放时段分布默认 30 天，周期独立可选 7 / 30 / 90 天或全部；按 24 小时桶展示播放事件时间分布，与观看历史 `played_at`、今日播放和趋势共用同一 canonical 时间字段（当前优先 `update_time`，再 fallback 其他已识别时间字段），使用 `TZ` 或默认 `Asia/Shanghai` 做本地时区分桶。
...
1. 报表总览。
2. 播放趋势热力图固定展示最近 1 年，按周列、周一/周三/周五标签、中文月份和即时 tooltip 展示。
3. 活跃用户榜独立选择统计周期，默认 30 天。
4. 热门媒体榜独立选择统计周期，默认 7 天，并支持单集/整部剧统计模式。
5. 媒体类型分布中文展示。
6. 分辨率分布，空值显示为“未记录”。
7. 模块刷新采用 stale-while-revalidate，失败时保留上次成功结果。
8. 播放时段分布独立选择统计周期，默认 30 天。
9. 收藏记录只读模块。
...
6. 飞牛数据库只读读取，不写入源库；可选 snapshot 只生成到 `/data/cache`。
...
- 数据中心活跃用户链接进入按用户筛选的观看历史，热门内容链接进入媒体库并打开对应详情。
- 顶部“功能”菜单从左侧滑出，不使用固定侧栏。
- 同一登录页面会话内缓存数据中心、观看历史、用户管理、媒体库四个页面，保留筛选、排序、分页、已加载记录和主内容滚动位置；页面重载后重新初始化。
- 缓存页面离开后暂停轮询和历史无限加载观察，返回后恢复；刷新通知仅交给当前页面。退出后台时销毁页面缓存。
