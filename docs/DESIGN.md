...
Phase 7A 先不启用报表缓存和任务化预计算。当前报表中心直接通过 SQLite 只读连接对飞牛源库做 SQL 聚合，返回基础统计结果；Phase 7B 将播放趋势整理为热力图，并增加热门媒体单集/整部剧统计模式。后续进入任务中心与缓存阶段时，再把可缓存报表写入 `admin.db` 或 `/data/cache`。
