...
Phase 7A/7B/7C 的报表 API 均需要登录鉴权，只读读取飞牛源库或可选快照，不写飞牛数据库。播放趋势、活跃用户、热门媒体、播放时段和分辨率分布在后端 SQL/服务层按时间范围聚合；播放趋势支持 7、30、90 和最多 365 天的 `all` 热力图，榜单、播放时段和分辨率分布支持 `all`，热门媒体支持 `episode` / `series` 统计模式，服务层会限制最大天数和最大 `limit`。收藏记录读取 `item_user_favorite`，下载记录读取 `download_task`，缺表时返回空状态和能力不可用。更复杂的完播率排行、弃剧排行和缓存预计算留到后续阶段。
...
1. 仅重做仪表盘首页视觉，不改变全局后台布局和其它业务页面。
2. 首页使用独立 `dashboard-monitor-page` 样式作用域，形成浅蓝渐变背景、居中容器和玻璃拟态监控面板风格。
3. 继续复用现有登录、导航、API 鉴权和只读查询链路。

模块：

1. 监控面板标题行：项目监控标题、当前时间、主题切换和刷新按钮。
2. 四个总览指标卡：总用户数、活跃用户、今日播放、播放记录。
3. 最近活跃观看：使用最近 5 分钟播放记录更新时间推断，明确标注非实时会话。
4. 播放时段：复用播放时段聚合数据，用轻量蓝色柱条展示，不引入新的大型图表库。
5. 观看历史：展示最近播放简版，包含用户、内容、类型、进度和时间。
6. 热门内容：展示热门内容前 10 条和播放次数。
7. 收藏记录：只读展示最近收藏，缺表或无数据时显示空状态。
8. 下载记录：只读展示最近下载，缺表或无数据时显示空状态。
